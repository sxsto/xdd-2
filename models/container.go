...
func (c *Container) write(cks []JdCookie) error {
	...
	case QL:
		if c.Version == "2.8" {
			if len(c.Delete) > 0 {
				_, err := c.request("/api/envs", DELETE, fmt.Sprintf(`[%s]`, strings.Join(c.Delete, ",")))
				if err != nil {
			...
			if len(cks) != 0 {
				...
				_, err := c.request("/api/envs", POST, fmt.Sprintf(`[%s]`, strings.Join(hh, ",")))
				if err != nil {
				...
				for k := range help {
					...
					if len(toDelete) > 0 {
						_, err := c.request("/api/envs", DELETE, fmt.Sprintf(`[%s]`, strings.Join(toDelete, ",")))
						if err != nil {
							log.Warn("删除青龙环境变量失败", err)
						}
					}
				...
				for k, v := range help {
					...
					d, _ := json.Marshal(r)
					_, err := c.request("/api/envs", POST, fmt.Sprintf(`[%s]`, string(d)))
					if err != nil {
						log.Warn("添加青龙环境变量失败", err)
					}
				}
			...
			if len(c.Delete) > 0 {
				_, err := c.request("/api/cookies", DELETE, fmt.Sprintf(`[%s]`, strings.Join(c.Delete, ",")))
				if err != nil {
					log.Warn("删除青龙环境变量失败", err)
				}
			}
			...
			if len(d) != 0 {
				_, err := c.request("/api/cookies", POST, fmt.Sprintf(`[%s]`, strings.Join(d, ",")))
				if err != nil {
					log.Warn("添加青龙环境变量失败", err)
				}
			}
