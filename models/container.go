...
func (c *Container) write(cks []JdCookie) error {
	...
	case QL:
		if c.Version == "2.8" || c.Version == "2.9" {
			if len(c.Delete) > 0 {
				_, err := c.request("/envs", DELETE, fmt.Sprintf(`[%s]`, strings.Join(c.Delete, ",")))
				if err != nil {
					log.Warn("删除青龙环境变量失败", err)
				}
			}
			var hh []string
			if len(cks) != 0 {
				...
				_, err := c.request("/envs", POST, fmt.Sprintf(`[%s]`, strings.Join(hh, ",")))
				if err != nil {
					log.Warn("添加青龙环境变量失败", err)
				}
				...
				for k := range help {
					var data, err = c.request("/envs?searchValue="+k, "", "")
					a := AutoGenerated{}
					...
					if len(toDelete) > 0 {
						_, err := c.request("/envs", DELETE, fmt.Sprintf(`[%s]`, strings.Join(toDelete, ",")))
						if err != nil {
							log.Warn("删除青龙环境变量失败", err)
						}
					}
				...
				for k, v := range help {
					...
					d, _ := json.Marshal(r)
					_, err := c.request("/envs", POST, fmt.Sprintf(`[%s]`, string(d)))
					if err != nil {
						log.Warn("添加青龙环境变量失败", err)
					}
				}
			...
			if len(c.Delete) > 0 {
				_, err := c.request("/cookies", DELETE, fmt.Sprintf(`[%s]`, strings.Join(c.Delete, ",")))
				if err != nil {
					log.Warn("删除青龙环境变量失败", err)
				}
			}
			...
			if len(d) != 0 {
				_, err := c.request("/cookies", POST, fmt.Sprintf(`[%s]`, strings.Join(d, ",")))
				if err != nil {
					log.Warn("添加青龙环境变量失败", err)
				}
			}
...
func (c *Container) read() error {
	...
	case QL:
		if c.Version == "2.8" || c.Version == "2.9" {
			...
			var data, err = c.request("/envs?searchValue=JD_COOKIE", "", "")
			a := AutoGenerated{}
			err = json.Unmarshal(data, &a)
			if err != nil {
				c.Available = false
			...
			var data, err = c.request("/cookies", "", "")
			if err != nil {
				c.Available = false
				return err
			}
	...
	for {
		...
		if data, err := req.Bytes(); err == nil {
			...
			if code == 200 {
				...
				if i >= 5 {
					return nil, errors.New("青龙接口请求失败 " + string(data))
				}
				_ = c.getToken()
			}
		}
...
func (c *Container) getConfig(handle func(*bufio.Reader) string) error {
	if c.Address == "" {
		...
		req := httplib.Get(c.Address + "/config/config")
...
func (c *Container) postConfig(handle func(config string) string) error {
	if c.Address == "" {
		...
		_, _ = f.WriteString(handle(c.Config))
	} else {
		req := httplib.Post(c.Address + "/save")
		req.Header("Cookie", c.Token)
